...
export const OutreachReviewModal = ({ open, drafts, onClose, onSend }: OutreachReviewModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/30 p-4">
      <div className="mx-auto mt-10 max-w-4xl rounded-2xl border border-slate-200 bg-white p-6 shadow-card">
        <div className="flex items-center justify-between gap-2">
          <h2 className="text-xl font-semibold text-ink">Outreach review</h2>
          <button type="button" onClick={onClose} className="btn-secondary">
            Close
          </button>
        </div>

        <p className="mt-2 text-sm text-slate-600">Tailored drafts are ready. Review and send in one click.</p>
        ...
        <div className="mt-5 flex justify-end">
          <button type="button" onClick={onSend} className="btn-primary px-5 py-2.5">
            Send outreach
          </button>
        </div>
      </div>
